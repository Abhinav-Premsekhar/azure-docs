...
> [!IMPORTANT]
> The Playwright Workspaces remote Model Context Protocol (MCP) server is in public preview. Preview features are provided without a service-level agreement and aren't recommended for production workloads. Features, limits, and availability can change.
...
To create a workspace access token see [Manage workspace access tokens](./how-to-manage-access-tokens).
...
A minimal tool allowlist for a read-oriented navigation flow is:
...
**Add Remote MCP connected screenshot.**
...
Prefer references returned by browser_snapshot or browser_find. They describe accessible page elements and reduce dependence on implementation-specific markup. A unique selector can be used when a stable snapshot reference isn't available.
...
An action can return a dialog-opened outcome after it has already changed page state. This outcome isn't a normal action failure.
...
All browser-scoped tools require browserSessionId. String and collection bounds shown here are current public-preview limits.
...
|---|---|---|
| browser_navigate | browserSessionId, url | Navigates to a URL. The URL can contain at most 16 KiB of characters. |
| browser_navigate_back | browserSessionId | Navigates to the previous entry in page history. |
| browser_snapshot | browserSessionId; optional target, depth, boxes | Returns an accessibility snapshot. target accepts a snapshot reference or unique selector. depth is 1-100. boxes includes bounding-box data when supported. Snapshot text is truncated at 64 KiB and marked as truncated. |
| browser_find | browserSessionId, text | Performs a case-insensitive substring search of the accessibility snapshot and returns matching snippets. text can contain at most 4,096 characters. |
...
| Tool | Parameters | Use and important behavior |
|---|---|---|
| browser_click | browserSessionId, target; optional button, doubleClick, modifiers | Clicks a target. button is left, right, or middle. Up to five modifiers are accepted from Alt, Control, ControlOrMeta, Meta, and Shift. Don't blindly retry a timed-out click. |
...
| browser_select_option | browserSessionId, target, values | Selects 1-100 option values. Each value can contain at most 4,096 characters. |
...
| browser_run_code | browserSessionId, code | Runs a Playwright JavaScript function body with access to the page. This is the highest-privilege tool and can produce arbitrary side effects. |
...
2. Remove test agents or project connections that aren't needed.
3. Revoke temporary workspace access tokens.
4. In Visual Studio Code, stop or remove the MCP server if the connection is no longer required.

Closing sessions promptly releases browser capacity and reduces the chance of unexpected activity or quota use.

## Related content

- [Manage workspace access tokens](./how-to-manage-access-tokens)
- [Manage access to a Playwright workspace](./how-to-manage-workspace-access)
